...
import net.imjeck.client.model.QuadProcessors;
		...
		try {
			for (String namespace : pack.getNamespaces(PackType.CLIENT_RESOURCES)) {
				try {
					...
					ContinuityClient.LOGGER.error("Failed to list CTM resources in namespace '" + namespace + "' from pack '" + pack.packId() + "'", e);
				}
			}
		} catch (Exception e) {
			ContinuityClient.LOGGER.error("Failed to enumerate namespaces from pack '" + pack.packId() + "'", e);
		}
	}
